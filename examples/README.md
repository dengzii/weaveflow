...
These examples use an OpenAI-compatible model. Configure `OPENAI_API_KEY`, `OPENAI_BASE_URL`, and `OPENAI_MODEL` in
the repository-root `.env` before running them. Never commit `.env` or `.local/` runtime data.
...
| `examples/supervisor_mode/graph.json`  | A complete editable Graph Definition for a compile-ready typed mini-language workflow.             |
