...
func (r *GraphRunner) Start(ctx context.Context, initialState State) (RunRecord, State, error) {
	...
	run := RunRecord{
		...
		EntryNodeID:  r.nodeID(r.Graph.entryPoint),
		StartedAt:    now,
		UpdatedAt:    now,
	}
	...
	return r.execute(ctx, run, initialState.CloneState(), r.Graph.entryPoint, nil, nil)
}
...
func (r *GraphRunner) execute(ctx context.Context, run RunRecord, state State, startNode string, skip *breakpointSkip, artifacts []ArtifactRef) (RunRecord, State, error) {
	...
	runnable, err := r.Graph.compileForRunner(execution)
	if err != nil {
...
func (r *GraphRunner) resumeTarget(checkpoint CheckpointRecord, state State) (string, *breakpointSkip, error) {
	nodeName, err := r.Graph.resolveNodeRef(checkpoint.NodeID)
	if err != nil {
		return "", nil, err
	}
	...
	case CheckpointAfterNode:
		nextName, err := r.resolveNextNode(nodeName, state)
...
func (r *GraphRunner) resolveNextNode(currentName string, state State) (string, error) {
	if conditional := r.Graph.conditionalEdges[currentName]; len(conditional) > 0 {
		for _, edge := range conditional {
			if edge.when(context.Background(), state) {
				return edge.to, nil
			}
		}
		if target, ok := r.Graph.edges[currentName]; ok {
			return target, nil
		}
		if currentName == r.Graph.finishPoint {
...
func (r *GraphRunner) notifyListeners(ctx context.Context, event langgraph.NodeEvent, nodeName string, state State, err error) {
	for _, listener := range r.Graph.globalListeners {
		listener.OnNodeEvent(ctx, event, nodeName, state, err)
	}
	for _, listener := range r.Graph.nodeListeners[nodeName] {
		listener.OnNodeEvent(ctx, event, nodeName, state, err)
	}
}
...
func (r *GraphRunner) nodeID(nodeName string) string {
	if spec, ok := r.Graph.nodeSpecs[nodeName]; ok && spec.ID != "" {
		return spec.ID
	}
	return nodeName
}
