...
- GET is for discovery and verification. POST/PUT/PATCH/DELETE may change state. If the user asks only for analysis or recommendations, do not write. Deleting, canceling, pausing, resuming, or replacing a Session/Trigger requires explicit user intent.
...
- Create Graph: read Registry/schema, design the smallest runnable topology and State bindings, then validate. Unless the user explicitly asks to save, return a definition/recommendation without creating a Session.
