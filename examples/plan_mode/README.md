...
- `-profile`: `tiny-script` (default), `coding-go`, `documentation`, or `analysis`.
...
The pre-hardening four-profile, two-run matrix is summarized in `REAL_MODEL_RESULTS.md`; it predates the grounded Critic and final evidence-reference requirements and is not evidence that those newer paths passed a live-provider run.
