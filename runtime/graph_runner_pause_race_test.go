...
func TestPauseRunAtCheckpointRetriesConflictAndPreservesChildOwner(t *testing.T) {
	t.Parallel()

	now := time.Unix(602, 0)
	requestKey := "pause-request"
	run := RunRecord{
		RunID: childRunIDForRequestKey(requestKey), ChildRequestKey: requestKey, ChildInputHash: "input-hash", ExecutionClaimID: "claim",
		ParentRunID: "parent", ParentStepID: "parent-step", ParentTaskID: "parent-task", RootRunID: "parent", RunPath: []string{"parent", childRunIDForRequestKey(requestKey)},
		GraphID: "graph", GraphVersion: "v1", Namespace: "root/child", Status: RunStatusRunning, PauseRequested: true, StartedAt: now, UpdatedAt: now,
	}
	store := NewMemoryRuntimeStore()
	createPauseTestChildRun(t, store, run, now)
	hookStore := &pauseHookTransactionStore{TransactionStore: store}
	hookStore.beforeFirstUpdate = func(ctx context.Context) error {
		persisted, err := store.GetRun(ctx, run.RunID)
		if err != nil {
	...
	runner := newPauseTestRunner(store, hookStore, now)
	ownerCtx := context.WithValue(context.Background(), childRunExecutionOwnerKey{}, run.ExecutionClaimID)

	canceled, _, err := runner.pauseRunAtCheckpoint(ownerCtx, run, state.NewState(), "checkpoint", nil, "")
	if err != nil {
		t.Fatalf("pauseRunAtCheckpoint() error = %v", err)
	}
	if canceled.Status != RunStatusCanceled || canceled.PauseRequested || canceled.CancelRequested || canceled.ExecutionClaimID != run.ExecutionClaimID {
		t.Fatalf("canceled child run = %#v", canceled)
	}
	if attempts := hookStore.updateAttempts.Load(); attempts != 2 {
		t.Fatalf("pause update attempts = %d, want 2", attempts)
	}
...
func TestPauseRetryRejectsChildClaimTakeover(t *testing.T) {
	t.Parallel()

	now := time.Unix(603, 0)
	requestKey := "takeover-request"
	run := RunRecord{
		RunID: childRunIDForRequestKey(requestKey), ChildRequestKey: requestKey, ChildInputHash: "input-hash", ExecutionClaimID: "old-claim",
		ParentRunID: "parent", ParentStepID: "parent-step", ParentTaskID: "parent-task", RootRunID: "parent", RunPath: []string{"parent", childRunIDForRequestKey(requestKey)},
		GraphID: "graph", GraphVersion: "v1", Namespace: "root/child", Status: RunStatusRunning, PauseRequested: true, StartedAt: now, UpdatedAt: now,
	}
	store := NewMemoryRuntimeStore()
	...
	hookStore.beforeFirstUpdate = func(ctx context.Context) error {
		...
		persisted.ExecutionClaimID = "new-claim"
		_, err = store.CompareAndSwapRun(ctx, persisted.Revision, persisted)
		return err
	}
	runner := newPauseTestRunner(store, hookStore, now)
	...
	latest, _, err := runner.pauseRunAtCheckpoint(ownerCtx, run, state.NewState(), "checkpoint", nil, "")
	if !errors.Is(err, ErrRunControlNotAllowed) {
		t.Fatalf("pauseRunAtCheckpoint() error = %v, want lost owner", err)
	}
	if latest.ExecutionClaimID != "new-claim" || latest.Status != RunStatusRunning {
		t.Fatalf("latest run = %#v", latest)
	}
	if attempts := hookStore.updateAttempts.Load(); attempts != 1 {
		t.Fatalf("pause update attempts = %d, want 1", attempts)
	}
	events, err := store.ListEvents(run.RunID)
	if err != nil {
		t.Fatalf("ListEvents() error = %v", err)
	...
	if len(events) != 0 {
		t.Fatalf("events after rejected takeover = %#v", events)
	}
}
