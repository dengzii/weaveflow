...
func TestTriggerRoutesReplaceListAndInvokeWebhook(t *testing.T) {
	...
	replaced := serveHTTP(engine, http.MethodPut, "/graphs/graph-1/triggers", `{"triggers":[{
		...
		"webhook":{"api_key":"secret","state_bindings":{"input":"scopes.webhook.input"},"state_mappings":[{"parameter":"user.id","state_path":"shared.user.id"}]}
	}]}`)
	if replaced.Code != http.StatusOK {
		t.Fatalf("replace status = %d, body = %s", replaced.Code, replaced.Body.String())
	}
	if strings.Contains(replaced.Body.String(), "secret") || strings.Contains(replaced.Body.String(), "api_key") {
		t.Fatalf("replace response leaked API key: %s", replaced.Body.String())
	}
	...
	unauthorized := serveHTTP(engine, http.MethodPost, "/graphs/graph-1/triggers/hook/invocations?api_key=secret", body)
	...
	invoked := requestWithHeader(engine, http.MethodPost, "/graphs/graph-1/triggers/hook/invocations", body, "Authorization", "Bearer secret")
	...
	webhook := requestWithHeader(engine, http.MethodPost, "/graphs/graph-1/triggers/hook/webhook", body, "Authorization", "Bearer secret")
	...
	if response := requestWithHeader(engine, http.MethodPost, "/graphs/graph-2/triggers/hook/webhook", body, "Authorization", "Bearer secret"); response.Code != http.StatusNotFound {
